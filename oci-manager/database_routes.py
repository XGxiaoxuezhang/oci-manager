...
def databases_overview():
    tenants = load_tenants()
    if tenants:
        return redirect(url_for("database.databases_home", tenant_name=sorted(tenants.keys())[0]))
    return render_page("databases_overview", module_title="数据库", module_path="databases", tenants=build_dashboard_cards(tenants))
...
def databases_home(tenant_name: str):
    ...
    try:
        ...
        return render_page(
            ...
            backups=[],
            database_stats={"total": 0, "available": 0, "free_tier": 0},
            database_form_defaults={"db_version": "19c", "cpu_core_count": 1, "storage_size_gbs": 20, "character_set": "AL32UTF8", "ncharacter_set": "AL16UTF16"},
            load_error=str(exc),
        )


@database_bp.route("/tenant/<tenant_name>/databases/export")
def databases_export(tenant_name: str):
    tenant_cfg = require_tenant(tenant_name)
    if tenant_cfg is None:
        return redirect(url_for("tenant.index"))
    try:
        context = run_with_timeout(8, list_autonomous_databases_context, tenant_cfg, None)
        return send_csv(
            f"{tenant_name}-databases.csv",
            ["display_name", "name", "workload", "cpu_core_count", "data_storage_size_in_tbs", "is_free_tier", "lifecycle_state", "created", "id"],
            context["autonomous_databases"],
        )
    except Exception as exc:
        flash(f"导出数据库失败：{exc}", "error")
...
def databases_create(tenant_name: str):
    ...
    try:
        ...
        return redirect(url_for("database.databases_home", tenant_name=tenant_name))
    if cpu_core_count < 1 or storage_size_gbs < 20:
        flash("CPU 至少 1 核，存储至少 20 GB。", "error")
        return redirect(url_for("database.databases_home", tenant_name=tenant_name))
...
@database_bp.route("/tenant/<tenant_name>/databases/<database_id>/backup", methods=["POST"])
def databases_backup(tenant_name: str, database_id: str):
    tenant_cfg = require_tenant(tenant_name)
    if tenant_cfg is None:
        return redirect(url_for("tenant.index"))
    try:
        backup_id = create_autonomous_database_backup(tenant_cfg, database_id, request.form.get("display_name", ""))
        flash(f"手动备份请求已提交：{backup_id or '-'}", "success")
    except Exception as exc:
        flash(f"创建手动备份失败：{exc}", "error")
    return redirect(url_for("database.databases_home", tenant_name=tenant_name, database_id=database_id))
